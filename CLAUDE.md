...
iOS Media Session lock-screen integration, progress bar). It is kept in sync
— same code — across three repos: this one, `original-bug-book`, and
`beyond-ice-and-steam`. The only intentional difference between copies is the
branding fallback near the bottom (`chapterTitle` / `album` in the Media
Session metadata), which should match the book it ships with.

If you update the playback logic in one copy, copy it to the other two and
only touch the branding lines.
